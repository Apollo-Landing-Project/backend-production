...
const getFileUrl = (file?: Express.Multer.File, folder: "images" | "files" = "images") =>
	file ? `${envConfig.host_url}/${folder}/${file.filename}` : null;
...
export class NewsNewsServices {
	static async create(
		...
		if (!titleEn) throw new Error("Title (EN) is required");
		...
		if (existingId) throw new Error("Title (ID) already exists in news");

		const existingReportId = await db.report.findFirst({
			where: { title_id: titleId },
		});
		if (existingReportId) throw new Error("Title (ID) already exists in reports");

		const existingEn = await db.newsNewsEn.findFirst({
			where: { title: titleEn },
		});
		if (existingEn) throw new Error("Title (EN) already exists in news");

		const existingReportEn = await db.report.findFirst({
			where: { title_en: titleEn },
		});
		if (existingReportEn) throw new Error("Title (EN) already exists in reports");
	...
	static async update(
		...
		const titleId =
			data.title === undefined ? undefined : normalizeTitle(data.title);
		const titleEn =
        ...
        // Handle report
        let reportUpdateData: any = undefined;
        let oldReportIdToDelete: string | null = null;
        let oldReportFileUrlToDelete: string | null = null;
		...
		if (titleId) {
			...
			if (duplicateId) throw new Error("Title (ID) already exists in news");
			...
			if (duplicateReportId) throw new Error("Title (ID) already exists in reports");
		}
		...
		if (titleEn) {
			...
			if (duplicateEn) throw new Error("Title (EN) already exists in news");
			...
			if (duplicateReportEn) throw new Error("Title (EN) already exists in reports");
		}
